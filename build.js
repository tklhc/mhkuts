...
if (babelUsed) {
  ...
  const prodTemplate = template
    ...
    .replace('<script type="text/babel" data-presets="react">', '<script>');
  output = prodTemplate.replace('{{SCRIPT_CONTENT}}', finalScript);
  console.log('🚀 Production build: Babel CDN kaldırıldı, React production min kullanıldı');
} else {
  output = template.replace('{{SCRIPT_CONTENT}}', finalScript);
}
